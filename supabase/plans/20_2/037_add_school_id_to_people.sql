-- DO NOT APPLY - DESIGN ONLY - SPRINT 20.2D
-- Proposed people wave after 036.
--
...
with family_candidates as (
  select
    relation.family_id,
    min(student.school_id::text)::uuid as school_id
  from public.student_families relation
  join public.students student on student.id = relation.student_id
  group by relation.family_id
  having count(distinct student.school_id) = 1
)
